...
function getAccess_Token(config, url, cb) {
    console.log('start access_token');
    // 判断内存中是否有缓存
    if (!token_cache || !token_cache.token) {
        console.log('readCache');
        readFile('token_cache.json', function(str) {
            if (str) {
                console.log(str);
                token_cache = JSON.parse(str);
            }
            tryGetToken(config, url, cb);
        });
    }
    else {
        tryGetToken(config, url, cb);
...
function tryGetToken(config, u, cb) {
    // 判断token 是否过期
    if (!token_cache.token || (new Date().getTime() - token_cache.time) > 7000000) {
        token_cache.waterfall([function(cb) {
            console.log('start getNew Ticket', token_cache);
            getToken(config, cb);
        }], function(error, result) {
            if (error) {
                cb('getToken getNewTicket error', error);
                ...
                token_cache.time = new Date().getTime();
                // 文件保存
                writeFile('token_cache.json', JSON.stringify(token_cache));
            }
        });
    }
    else {
        var token = token_cache.token;
    }
}
...
exports.getAccess_Token = function(config) {
    return function(url, cb) {
        getAccess_Token(config, url, cb);
    }
};
